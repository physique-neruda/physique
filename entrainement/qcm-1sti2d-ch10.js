/* Genere par outils/construire.py — ne pas editer a la main.
   1sti2d · chapitre 10 · Énergie chimique
   Les QCM viennent de ch10_prerequis.tex et ch10_bilan.tex,
   les cartes de cartes/cartes-1sti2d-ch10.json. */
window.CHAPITRE = {
 "filiere": "1sti2d",
 "num": "10",
 "titre": "Énergie chimique",
 "niveau": "1re STI2D",
 "prerequis": [
  {
   "q": "Quelle est l'unité de l'énergie dans le Système international ?",
   "choix": [
    "le watt (W)",
    "le joule (J)",
    "le kelvin (K)",
    "le newton (N)"
   ],
   "bonne": 1,
   "expl": "le joule (J)"
  },
  {
   "q": "Un convertisseur fournit une puissance de 2000 W pendant 30 s. L'énergie mise en jeu (E = P × Δt) vaut :",
   "choix": [
    "66 J",
    "2030 J",
    "60 kJ",
    "60 MJ"
   ],
   "bonne": 2,
   "expl": "E = 2000 × 30 = 60 000 J = 60 kJ (Δt en secondes)"
  },
  {
   "q": "À combien de joules correspond 1 kWh ?",
   "choix": [
    "3,6 MJ",
    "1000 J",
    "3600 MJ",
    "60 J"
   ],
   "bonne": 0,
   "expl": "1 kWh = 3,6 MJ"
  },
  {
   "q": "Pour l'eau, la capacité thermique massique vaut c ≈ 4180 J/kg/K. L'énergie à fournir pour chauffer 1 kg d'eau de 20 °C à 30 °C (Q = m c Δθ) vaut environ :",
   "choix": [
    "42 J",
    "42 kJ",
    "418 J",
    "4180 kJ"
   ],
   "bonne": 1,
   "expl": "Q = 1 × 4180 × 10 = 41 800 J ≈ 42 kJ"
  },
  {
   "q": "Deux corps de températures différentes sont mis en contact. Spontanément, le transfert thermique se fait :",
   "choix": [
    "du corps froid vers le corps chaud",
    "du corps chaud vers le corps froid",
    "dans les deux sens en même temps",
    "il n'y a aucun transfert"
   ],
   "bonne": 1,
   "expl": "du chaud vers le froid"
  },
  {
   "q": "Une température de 25 °C correspond, en kelvins, à :",
   "choix": [
    "25 K",
    "248 K",
    "298 K",
    "273 K"
   ],
   "bonne": 2,
   "expl": "T = 25 + 273 = 298 K"
  },
  {
   "q": "L'énergie libérée par un combustible est proportionnelle à sa masse (E = PC × m). Si l'on double la masse de combustible, l'énergie libérée :",
   "choix": [
    "ne change pas",
    "est divisée par deux",
    "double",
    "est multipliée par quatre"
   ],
   "bonne": 2,
   "expl": "proportionnalité : masse × 2 ⇒ énergie × 2"
  }
 ],
 "bilan": [
  {
   "q": "Le <strong>système chimique</strong>, c'est :",
   "choix": [
    "l'air qui entoure la réaction",
    "l'ensemble des espèces qui réagissent et se forment",
    "le récipient dans lequel on travaille",
    "la source de chaleur"
   ],
   "bonne": 1,
   "expl": ""
  },
  {
   "q": "L'<strong>effet thermique</strong> d'une transformation désigne :",
   "choix": [
    "la température des réactifs",
    "le transfert d'énergie thermique qui l'accompagne",
    "la masse des produits formés",
    "la durée de la réaction"
   ],
   "bonne": 1,
   "expl": ""
  },
  {
   "q": "Lors d'une transformation <strong>exothermique</strong>, l'énergie du système :",
   "choix": [
    "augmente",
    "diminue",
    "reste constante",
    "devient nulle"
   ],
   "bonne": 1,
   "expl": "exo : le système <em>cède</em>, son énergie diminue"
  },
  {
   "q": "Toutes les <strong>combustions</strong> sont :",
   "choix": [
    "endothermiques",
    "exothermiques",
    "tantôt l'une tantôt l'autre",
    "sans effet thermique"
   ],
   "bonne": 1,
   "expl": ""
  },
  {
   "q": "Le <strong>triangle du feu</strong> réunit :",
   "choix": [
    "combustible, comburant, énergie d'activation",
    "chaleur, lumière, fumée",
    "oxygène, azote, carbone",
    "combustible, eau, air"
   ],
   "bonne": 0,
   "expl": ""
  },
  {
   "q": "Dans une combustion, le <strong>comburant</strong> est le plus souvent :",
   "choix": [
    "le dioxyde de carbone CO₂",
    "le dioxygène O₂ de l'air",
    "le diazote N₂",
    "la vapeur d'eau"
   ],
   "bonne": 1,
   "expl": ""
  },
  {
   "q": "L'<strong>énergie d'activation</strong> sert à :",
   "choix": [
    "entretenir la combustion jusqu'à la fin",
    "amorcer la réaction",
    "refroidir les produits",
    "augmenter le pouvoir calorifique"
   ],
   "bonne": 1,
   "expl": "amorcer (c'est l'auto-entretien qui prend le relais ensuite)"
  },
  {
   "q": "Une combustion <strong>s'auto-entretient</strong> parce que :",
   "choix": [
    "elle n'a besoin d'aucun réactif",
    "l'énergie qu'elle libère porte le combustible voisin à sa température d'inflammation",
    "le comburant se régénère",
    "la température du système diminue"
   ],
   "bonne": 1,
   "expl": ""
  },
  {
   "q": "Le <strong>pouvoir calorifique</strong> s'exprime en :",
   "choix": [
    "MJ",
    "MJ/kg",
    "W",
    "kg/MJ"
   ],
   "bonne": 1,
   "expl": ""
  },
  {
   "q": "Un combustible a PC = 50 MJ/kg. L'énergie libérée par 3,0 kg vaut :",
   "choix": [
    "17 MJ",
    "53 MJ",
    "150 MJ",
    "150 kJ"
   ],
   "bonne": 2,
   "expl": "E = PC × m = 50 × 3,0 = 150 MJ"
  },
  {
   "q": "Une combustion <strong>incomplète</strong> produit, en plus de CO₂ et H₂O :",
   "choix": [
    "du dioxygène",
    "du monoxyde de carbone CO et des suies",
    "du diazote",
    "rien de particulier"
   ],
   "bonne": 1,
   "expl": ""
  },
  {
   "q": "Le monoxyde de carbone est particulièrement dangereux parce qu'il est :",
   "choix": [
    "coloré et odorant",
    "incolore, inodore et toxique",
    "plus léger que l'air et inoffensif",
    "visible sous forme de fumée noire"
   ],
   "bonne": 1,
   "expl": ""
  }
 ],
 "cartes": [
  {
   "type": "trou",
   "recto": "Règle sans exception : exothermique → énergie du système qui <strong>diminue</strong> ; endothermique → énergie qui …….",
   "verso": "<strong>augmente</strong>",
   "origine": "cours a completer"
  },
  {
   "type": "trou",
   "recto": "Elle fait intervenir le …… (ce qui brûle) et le <strong>comburant</strong> (le O₂ de l'air).",
   "verso": "<strong>combustible</strong>",
   "origine": "cours a completer"
  },
  {
   "type": "trou",
   "recto": "Pourquoi gratter une allumette ? Parce que la réaction doit franchir une « bosse » : il faut lui fournir une …… pour l'amorcer.",
   "verso": "<strong>énergie d'activation</strong>",
   "origine": "cours a completer"
  },
  {
   "type": "trou",
   "recto": "Le …… PC est l'énergie libérée par la combustion complète de <strong>1 kg</strong> (ou 1 L) de combustible, en kJ/kg ou MJ/kg.",
   "verso": "<strong>pouvoir calorifique</strong>",
   "origine": "cours a completer"
  },
  {
   "type": "trou",
   "recto": "Deux détecteurs sauvent des vies : <strong>fumée</strong> (obligatoire) et …….",
   "verso": "<strong>monoxyde de carbone</strong>",
   "origine": "cours a completer"
  }
 ]
};
